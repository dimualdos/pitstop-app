...
const Page: FC<{ params: { servises: string } }> = ({ params }) => {
    ...
    console.log(page?.path, `/${params.servises}`);
    ...
    }, [nameCarArr, page, params.servises]);
            ...
            {page && (
                <div className='flex  gap-4 flex-col pl-2 pr-2 items-center '>
                    <div className=' text-center'>
                        <h1 role="heading" className='text-sm md:text-[18px] dark:text-stone-300 tracking-wider uppercase font-semibold'>
                            {page?.H1}
                        </h1>
                            ...
                            {page.imagesArray && page.imagesArray.length > 0 && page.imagesArray.filter((obj: IimagesArr, indexObj: number) => JSON.stringify(obj) !== '{}' && obj.auto && obj.repairArray)
                                .map((item, indexImage) => {
                                    ...
                                    return <button
                                        ...
                                        className={clsx(
                                            'h-[100%] mr-1 p-0.5 sm:p-1 text-[8px] sm:text-xs border-[#AE4A84] border-solid border-2 rounded bg-slate-300/50 hover:bg-stone-300  hover:text-[#AE4A84] dark:text-stone-200 ',
                    ...
                    <div className='flex flex-col w-full pl-6'>
                        {page.liFirstDiv && page.liFirstDiv.map((li, indexLi) => <ul key={indexLi} className=' list-disc  dark:text-stone-300 '>
                            <li className='tracking-wider text-xs md:text-base'>{li}</li>
                        </ul>)}
                    </div>
